...
static long double compute_coeff_scale(polynomial_t *P) {
  ...
  for (size_t i = 0; i <= n; i++) {
    long double x = cxabsl(P->coeffs[i]);
...
static long double inner_root_radius(polynomial_t *P) {
  ...
  long double a0 = cxabsl(P->coeffs[0]);
  ...
  for (int iter = 0; iter < NEWTON_ITERS; iter++) {
    ...
    for (size_t i = 0; i < n; i++) {
      f += cxabsl(P->coeffs[i]) * beta_pow;
      if (i > 0) df += cxabsl(P->coeffs[i]) * (long double)i * powl(beta, (long double) (i - 1));
      beta_pow *= beta;
    }
...
long double polynomial_scale_for_roots(polynomial_t *P) {
  ...
  if (coeff_scale != 1.0l) {
    for (size_t i = 0; i <= n; i++) {
      P->coeffs[i] = cxscalel(P->coeffs[i], coeff_scale);
    }
  }
  ...
  if (sigma != 1.0l) {
    ...
    long double const_mag = cxabsl(P->coeffs[0]);
    long double lead_mag = cxabsl(P->coeffs[n]);
  ...
  // apply variable scaling coeff[i] *= sigma^i
  if (sigma != 1.0l) {
    long double sigma_pow = 1.0l;
    for (size_t i = 0; i <= n; i++) {
      P->coeffs[i] = cxscalel(P->coeffs[i], sigma_pow);
      sigma_pow *= sigma;
    }
  }
...
jt_status H_norm_next(polynomial_t *H, polynomial_t *P, cxldouble *k_p, cxldouble *k_h,
  ...
  cxldouble b_p = P->coeffs[p_deg];
  for (size_t i = p_deg; i > 0; i--) {
    k_p[i - 1] = b_p;
    b_p = cxaddl(P->coeffs[i - 1], cxmull(b_p, shift));
  } // b_p = P(shift)

  cxldouble b_h = H->coeffs[h_deg];
  for (size_t i = h_deg; i > 0; i--) {
    k_h[i - 1] = b_h;
    b_h = cxaddl(H->coeffs[i - 1], cxmull(b_h, shift));
  } // b_h = H(shift)

  // check if found a root
  if (cxabsl(b_p) < JT_EPSILON * (long double)p_deg) {
    *H_out = NULL;
  ...
  cxldouble norm = cxmull(cxnegl(b_h), k_p[p_deg - 1]);
  if (cxabsl(norm) < JT_EPSILON * cxabsl(k_p[p_deg - 1])) {
    *H_out = NULL;
    return JT_H_ZERO;
  }
  ...
  k_p[p_deg-1] = CXL(1.0, 0.0);
  for (size_t i = 0; i < p_deg-1; i++) { // h is one degree smaller
    // (k_h[i] * b_p - b_h * k_p[i]) / norm
    cxldouble num = cxsubl(cxmull(k_h[i], b_p), cxmull(b_h, k_p[i]));
    k_p[i] = cxdivl(num, norm);
  }

  for (size_t i = 0; i < p_deg; i++) {
    if (!isfinite(cxreall(k_p[i])) || !isfinite(cximagl(k_p[i]))) {
  ...
  if (next_shift) {
    cxldouble h_at_s = polynomial_eval(hnext, shift);
    if (cxabsl(h_at_s) > JT_EPSILON * (long double)(p_deg - 1)) {
      // shift - b_p / h_at_s
      *next_shift = cxsubl(shift, cxdivl(b_p, h_at_s));
    } else {
      *next_shift = shift;
    }
  }
...
jt_status find_next_root(polynomial_t *P, cxldouble *root_out) {
  ...
  cxldouble *work_p = calloc(P->degree, sizeof(cxldouble));
  cxldouble *work_h = calloc(P->degree, sizeof(cxldouble));
  if (!work_p || !work_h) {
  ...
  // degree 1 base case
  if (P->degree == 1) {
    *root_out = cxdivl(cxnegl(P->coeffs[0]), P->coeffs[1]);
    free(work_p);
    free(work_h);
    return JT_OK;
  }
  ...
  if (P->degree == 2) {
    cxldouble a = P->coeffs[2];
    cxldouble b = P->coeffs[1];
    cxldouble c = P->coeffs[0];
    // disc = sqrt(b*b - 4*a*c)
    ...
    cxldouble b_plus_disc = cxaddl(b, disc);
    cxldouble b_minus_disc = cxsubl(b, disc);
    cxldouble q;
    if (cxabsl(b_plus_disc) > cxabsl(b_minus_disc)) {
      q = cxscalel(b_plus_disc, -0.5L);
    } else {
      q = cxscalel(b_minus_disc, -0.5L);
    }

    cxldouble r1 = cxdivl(q, a);
    cxldouble r2 = cxdivl(c, q);

    // return smaller magnitude root first (Jenkins-Traub convention)
    *root_out = (cxabsl(r1) < cxabsl(r2)) ? r1 : r2;
  ...
  for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    // s = beta * exp(i * angle)
    cxldouble s = cxscalel(cxexpl(CXL(0, angle)), beta);
    ...
    for (int i = 0; i < STAGE1_ITERS; i++) {
      polynomial_t *H_new;
      jt_status st = H_norm_next(H, P, work_p, work_h, CXL(0, 0), NULL, &H_new);
      polynomial_free(H);
      if (st == JT_ERROR) { H = NULL; break; }
    ...
    for (size_t i = 0; i < max_iters; i++) {
      polynomial_t *H_new;
      jt_status st = H_norm_next(H, P, work_p, work_h, s, NULL, &H_new);
      ...
      if (st == JT_H_ZERO) {
        // perturb and retry
        // s = s * 1.001 + 0.001 * exp(i * h_zero_count)
...
polynomial_t *polynomial_deflate(polynomial_t *P, cxldouble root) {
  ...
  cxldouble *a = P->coeffs;
  cxldouble *b = calloc(p_deg, sizeof(cxldouble));
  if (!b) {
    return NULL;
  }
  ...
  for (size_t k = p_deg-1; k > 0; k--) {
    b[k - 1] = cxaddl(a[k], cxmull(root, b[k]));
  } // b_n = P(root)
...
static cxldouble polish_root(polynomial_t *P, cxldouble root) {
  ...
  for (int iter = 0; iter < POLISH_ITERS; iter++) {
    ...
    for (size_t i = P->degree; i > 0; i--) {
      ...
      p_val = cxaddl(P->coeffs[i-1], cxmull(p_val, z));
    }
...
bool polynomial_find_roots_scaled(polynomial_t *poly, cxldouble *roots, size_t *num_roots) {
  ...
  for (size_t i = 0; i <= work->degree; i++) {
    long double m = cxabsl(work->coeffs[i]);
    if (m > orig_scale) orig_scale = m;
  }
  long double zero_tol = orig_scale * LDBL_EPSILON * 10.0l;  // conservative tolerance

  while (work->degree > 0 && cxabsl(work->coeffs[0]) < zero_tol) {
    roots[roots_found++] = CXL(0.0, 0.0);
  ...
  // make monic
  cxldouble lead = work->coeffs[work->degree];
  for (size_t i = 0; i <= work->degree; i++) {
    work->coeffs[i] = cxdivl(work->coeffs[i], lead);
  }

  while (work->degree > 0) {
    cxldouble scaled_root;
